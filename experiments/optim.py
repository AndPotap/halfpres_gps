...
def fit_gpytorch_minibatch(
    ...
    batch_size = optim_options.pop("batch_size", None)
    ...
    train_inputs, train_targets = mll.model.train_inputs[0], mll.model.train_targets
    if batch_size is None:
        batch_size = int(train_inputs.shape[-2] / 4) # reasonable heuristic
    dataset = TensorDataset(train_inputs, train_targets)
    dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True)

    while not stop:
        epoch_loss = 0.
        for train_input, train_target in dataloader:
            optimizer.zero_grad()
            with gpt_settings.fast_computations(log_prob=approx_mll):
                output = mll.model(train_input)
                # we sum here to support batch mode
                args = [output, train_target] + _get_extra_mll_args(mll)
                loss = -mll(*args).sum()
                loss.backward()
        ...
        stop = stopping_criterion.evaluate(fvals=epoch_loss.detach())
    info_dict = {
