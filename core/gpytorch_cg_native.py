...
class GPyTorchNativeCG:
    ...
    def run_mbcg_with_tracking(self):
        ...
        state = initialize_native(self.A, rhs, self.x0)
...
def linear_cg_native(
    ...
    state = initialize_native(matmul_closure, rhs, preconditioner, x0)
...
def take_cg_step_log_native(state, A):
    ...
    alpha, denom = update_alpha_native(r0, p0, Ap0, has_converged)
    x1 = x0 + alpha * p0
    r1 = r0 - alpha * Ap0
    log_gamma1 = torch.tensor(0., dtype=x0.dtype)
    beta = update_beta_native(r1, Ap0, denom, has_converged)
    p1 = r1 + beta * p0
...
def update_alpha_native(r, p, Ap, has_converged):
    num = torch.sum(r * p, dim=-2)
    denom = torch.sum(p * Ap, dim=-2)
    alpha = do_safe_div(num, denom)
    alpha = torch.where(has_converged, torch.zeros_like(alpha), alpha)
    return alpha, denom


def update_beta_native(r, Ap, denom, has_converged):
    num = -torch.sum(r * Ap, dim=-2)
    beta = do_safe_div(num, denom)
    beta = torch.where(has_converged, torch.zeros_like(beta), beta)
    return beta


def do_safe_div(num, denom):
    is_zero = torch.abs(denom) < torch.tensor(1.e-10, dtype=num.dtype)
    denom = torch.where(is_zero, torch.ones_like(denom), denom)
    output = num / denom
    output = torch.where(is_zero, torch.zeros_like(output), output)
    return output
